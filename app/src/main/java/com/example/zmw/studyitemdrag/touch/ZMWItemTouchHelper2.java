...
public class ZMWItemTouchHelper2 extends RecyclerView.ItemDecoration implements RecyclerView.OnChildAttachStateChangeListener {
    ...
    //当用户将view滑动到边缘的时候，我们开始滑动LayoutManager，直到停止滑动或者将view部分划出了屏幕
    ...
    //re-used list for selecting a swap target,复用的列表用来选择交换的目标view
    ...
    private final OnItemTouchListener mOnItemTouchListener = new OnItemTouchListener() {
        ...
        public boolean onInterceptTouchEvent(RecyclerView rv, MotionEvent event) {
            ...
            final int action = MotionEventCompat.getActionMasked(event);//获取action
            if (action == MotionEvent.ACTION_DOWN) {
                mActivePointerId = MotionEventCompat.getPointerId(event, 0);//获取MotionEvent的id
    ...
    public ZMWItemTouchHelper2(Callback callback) {
        mCallback = callback;
    }

    //返回true表示点击在这个view中，否则在view外
    ...
    private void select(RecyclerView.ViewHolder selected, int actionState) {
        ...
        if (mSelected != null) {
            ...
            if(prevSelected.itemView.getParent()!=null){
                ...
                final RecoverAnimation ra=new RecoverAnimation(prevSelected,animationType,prevActionState,currentTranslateX,
...
//    列表靠前的就是离当前触碰区域最近的view
    ...
    private void moveIfNecessary(RecyclerView.ViewHolder viewHolder){
        ...
        final float threshold = mCallback.getMoveThreshold(viewHolder);//移动的门槛
    ...
    public void onChildViewDetachedFromWindow(View view) {
        ...
        if(mSelected!=null&&mSelected==holder){
            select(null, ACTION_STATE_IDLE);
    ...
    private RecyclerView.ViewHolder findSwipeView(MotionEvent motionEvent){
        final RecyclerView.LayoutManager lm = mRecyclerView.getLayoutManager();
        if (mActivePointerId == ACTIVE_POINTER_ID_NONE) {
            return null;
        }
